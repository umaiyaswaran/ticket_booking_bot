...
import db
import auth
from qr_generator import generate_booking_qr, generate_ticket_html, generate_ticket_pdf
...
<style>
    ...
    .profile-card h2 { margin: 0; color: white; font-family: 'Inter', sans-serif; font-weight: 700; }
    ...
    .stat-box h3 { margin: 0; color: #000000; }
    .stat-box p { margin: 5px 0 0 0; color: #5e5e5e; font-size: 0.9em; }
</style>
...
# =====================================================
from agency_sidebar import render_user_sidebar
render_user_sidebar()
                ...
                if booking.get("status") != "cancelled":
                    ...
                    with btn_col1:
                        try:
                            ...
                            pdf_data = generate_ticket_pdf(booking, agency_info, payment_record)
                            if isinstance(pdf_data, bytearray):
                                pdf_data = bytes(pdf_data)
                            st.download_button(
                                label="📥 Download PDF",
                                data=pdf_data,
                                file_name=f"TicketHub_{booking['booking_id']}.pdf",
                                mime="application/pdf",
                                key=f"dl_pdf_{booking['booking_id']}",
                    ...
                    with btn_col2:
                        try:
                            ...
                            if st.button("📱 Send to WhatsApp", key=f"wa_{booking['booking_id']}", use_container_width=True):
                                wa_result = notif_manager.send_booking_confirmation(booking)
                                if wa_result.get("customer_sent") or wa_result.get("pdf_sent"):
                                    st.success("Sent!")
                                else:
                    ...
                    with btn_col3:
                        if st.button("❌ Cancel", key=f"cancel_{booking.get('booking_id')}", use_container_width=True):
