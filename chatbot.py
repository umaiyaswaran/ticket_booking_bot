...
def extract_date(text):
    """Extract travel date from text - supports multiple voice/text formats"""
    text_lower = text.lower()
    ...
    # Pattern 4: "Dth Month YYYY" or "Month Dth YYYY" (e.g., "2nd june 2026", "june 2nd 2026", "27 july 2026")
    # Case A: "2nd june 2026" or "2 june 2026"
    for month_name, month_num in months_map.items():
        # "D Month YYYY" or "Dth Month YYYY"
        pattern = rf'(\d{{1,2}}){ordinal_suffix}?\s+{month_name}\s+(\d{{4}})'
        ...
        pattern = rf'{month_name}\s+(\d{{1,2}}){ordinal_suffix}?\s+(\d{{4}})'
        match = re.search(pattern, text_lower)
        if match:
            day = match.group(1).zfill(2)
            year = match.group(2)
    ...
    # Pattern 5: "DD Month" or "Month DD" (assume current year if not provided)
    current_year = datetime.now().year
    for month_name, month_num in months_map.items():
        # "D Month" or "Dth Month"
        pattern = rf'(\d{{1,2}}){ordinal_suffix}?\s+{month_name}'
        match = re.search(pattern, text_lower)
        if match:
            day = match.group(1).zfill(2)
            month = str(month_num).zfill(2)
        ...
        pattern = rf'{month_name}\s+(\d{{1,2}}){ordinal_suffix}?'
        match = re.search(pattern, text_lower)
        if match:
            day = match.group(1).zfill(2)
            month = str(month_num).zfill(2)
    ...
    # Pattern 6: "DD MM YYYY" (space-separated, e.g., "2 7 2026" or "27 07 2026")
    ...
    if match:
        ...
        # Determine day/month order (assume DD MM YYYY or MM DD YYYY)
        if 1 <= part1 <= 12 and 1 <= part2 <= 31:
            # Ambiguous: try DD MM first (more common)
            day = str(part1).zfill(2)
            month = str(part2).zfill(2)
        elif 1 <= part1 <= 31 and 1 <= part2 <= 12:
            day = str(part1).zfill(2)
            month = str(part2).zfill(2)
        else:
            day = str(part1).zfill(2)
    ...
    # Pattern 7: Just "DD MM" (assume current year, e.g., "27 07")
    match = re.search(r'\b(\d{1,2})\s+(\d{1,2})\b', text)
    if match:
        part1 = int(match.group(1))
        part2 = int(match.group(2))
    ...
    # Pattern 8: "DD YYYY" (e.g., "27 2026" - missing month, return None to prompt user)
    match = re.search(r'\b(\d{1,2})\s+(\d{4})\b', text)
    if match:
        day = int(match.group(1))
        year = match.group(2)
        if 1 <= day <= 31 and 1900 <= int(year) <= 2100:
            # Ambiguous: day + year but no month - return None so chatbot asks
            return None
...
def handle_route_inquiry(conv, user_message):
    ...
    If the date is missing, explain that you can accept these formats:
    - Spoken: "2nd June 2026" or "July 2nd 2026" or "27 July 2026"
    - Numeric: "2/7/2026" or "2-7-2026" or "2026-07-02"
    - Simple: "2 7 2026" (day month year)
...
def handle_agency_selection(conv, user_message, username=None):
    ...
    # Extract number from various formats: "2", "option 2", "number 2", "2nd", "option two", etc.
    ...
    # Try "option N", "number N", "Nth option", etc.
    match = re.search(r'(?:option|number|agency|choose|select|pick)?\s*(\d{1,2})\s*(?:st|nd|rd|th)?\s*(?:option)?', text_lower)
    if match:
        try:
    ...
    # Try word numbers: "option two", "second option"
    for word, num in number_words.items():
        if word in text_lower:
            # Check if it's preceded by "option" or is standalone
            idx = num - 1
            if 0 <= idx < len(conv.available_agencies):
                # Make sure it's not just a word in the agency name
                if re.search(rf'(?:option|number|agency|choose|select|pick)\s*{word}\b', text_lower) or \
                   re.search(rf'\b{word}\s*(?:option|agency)?\s*$', text_lower):
                    conv.selected_agency = conv.available_agencies[idx]['agency_username']
                    return get_available_seats_message(conv, username)
...
def handle_booking_confirmation(conv, user_message, username):
    ...
    if 'yes' in text_lower or 'confirm' in text_lower or 'book' in text_lower:
        # Calculate fare before redirecting to payment
        from payments import calculate_fare
        fare = calculate_fare(conv.source, conv.destination, seats=1, agency_username=conv.selected_agency)
        print(f"[CHATBOT] calculate_fare src={conv.source}, dest={conv.destination}, agency={conv.selected_agency}, fare={fare}")
        ...
        st.session_state.payment_details = {
            ...
            "travel_date": conv.date,
            "agency": conv.selected_agency,
            "seat": conv.selected_seat,
            "passenger_name": conv.passenger_name,
            "passenger_age": conv.passenger_age,
            "passenger_gender": conv.passenger_gender,
            "fare": fare,
            "username": username,
        }
        ...
        response = f"""
✅ **BOOKING SUMMARY — PAYMENT REQUIRED**
━━━━━━━━━━━━━━━━━━━━━━━━
📍 Route: {conv.source} → {conv.destination}
📅 Date: {conv.date}
💺 Seat: {conv.selected_seat}
...
def process_message(user_message, username="user", is_voice=False):
    """
    ...
    Args:
        ...
        is_voice: True if the message was captured via voice input
    """
    # Log voice input to database
    if is_voice:
        try:
            db.log_voice_message(username, user_message, is_voice_input=True, is_voice_output=False)
        except Exception:
            pass
...
🎫 **BOOK A TICKET**
Say: "book Chennai to Madurai on 2nd June 2026"
Or: "book Delhi to Mumbai on 13/07/2026"
Or: "book Chennai to Madurai 2 7 2026"

📅 **DATE FORMATS ACCEPTED:**
• "2nd June 2026" / "June 2nd 2026"
• "27 July 2026" / "July 27 2026"
• "2/7/2026" or "2-7-2026"
• "2 7 2026" (day month year)
